...
export const TransactionDetailPage = () => {
  ...
  const relatedGoalId = typeof transaction?.goal === 'string' ? transaction.goal : transaction?.goal?._id || transaction?.goal?.gid || transaction?.goalId || transaction?.gid;
  const { goal: relatedGoal } = useGoalDetail(relatedGoalId);
  const transactionType = transaction && TRANSACTION_TYPE[transaction.type];
  const transactionDate = getTransactionDate(transaction);
  const relatedGoalName = relatedGoal?.name || transaction?.goal?.name || transaction?.goalName || transaction?.goal?.goalName || 'Meta no disponible';
