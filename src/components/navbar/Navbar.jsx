      ...
      <div className='navbar-container'>
        ...
        <div className='navbar-actions' ref={menuRef}>
          {isAuthenticated ? <>
            ...
            {isMenuOpen && <div className='profile-menu'><button type='button' onClick={() => goTo('/goal')}><UserRound size={16} aria-hidden='true' /> Mi espacio</button><button type='button' onClick={() => goTo('/goals')}><UserRound size={16} aria-hidden='true' /> Mis metas</button><button type='button' onClick={() => goTo('/transactions')}><History size={16} aria-hidden='true' /> Transacciones</button><button type='button' onClick={() => goTo('/financial')}><BriefcaseBusiness size={16} aria-hidden='true' /> Situación financiera</button><button type='button' onClick={() => goTo('/income')}><Wallet size={16} aria-hidden='true' /> Ingresos</button><button type='button' onClick={handleLogout} className='logout-action'><LogOut size={16} aria-hidden='true' /> Cerrar sesión</button></div>}
