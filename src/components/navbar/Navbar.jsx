    ...
    <header className='navbar'>
      <div className='navbar-container'>
        <button className='navbar-brand' type='button' onClick={() => goTo('/')} aria-label='Ir al inicio'>
          <span className='brand-mark'>ah</span><span className='navbar-title'>ahorra<span>Hoy</span></span>
        </button>
        ...
        <div className='navbar-actions' ref={menuRef}>
          {isAuthenticated ? <>
            ...
            {isMenuOpen && <div className='profile-menu'><button type='button' onClick={() => goTo('/')}><UserRound size={16} aria-hidden='true' /> Mi espacio</button><button type='button' onClick={handleLogout} className='logout-action'><LogOut size={16} aria-hidden='true' /> Cerrar sesión</button></div>}
